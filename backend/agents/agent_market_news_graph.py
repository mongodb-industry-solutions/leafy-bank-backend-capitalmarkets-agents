...
from agents.tools.tool_asset_news_sentiments import generate_news_summaries_tool
...
def create_workflow_graph(checkpointer=None):
    ...
    graph.add_node("asset_news_sentiments_node", generate_news_summaries_tool)
    
    # Define the edges
    graph.add_edge("portfolio_allocation_node", "fetch_market_news_node")
    graph.add_edge("fetch_market_news_node", "asset_news_sentiments_node")
    graph.add_edge("asset_news_sentiments_node", END)
