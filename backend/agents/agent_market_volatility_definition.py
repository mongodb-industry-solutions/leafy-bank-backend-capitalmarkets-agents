...
class MarketVolatilityAgentDefinition:
    def __init__(self, llm, tools):
        self.llm = llm
        self.tools = tools
        self.system_message = (
            "You are a Market Volatility Agent responsible for evaluating the Market Volatility Index (VIX indicator)."
            " Daily changes in the VIX (Volatility Index) often referred to as the fear gauge."
            " A VIX baseline generally refers to a considered normal range for the Volatility Index (VIX), which typically falls between 12 and 20."
            " Use the provided tools to assess the fluctuations and provide a recommended action based on your analysis."
            " The output should be in JSON format with the following structure:"
            "\n{{"
            "\n  \"indicator\": \"(Market Volatility symbol here)\","
...
llm = get_llm()
logger.info("Market Volatility Agent LLM loaded successfully.")
market_volatility_agent = MarketVolatilityAgentDefinition(llm, tools)
chatbot_agent = market_volatility_agent.get_agent()
logger.info("Market Volatility Agent created and ready to use.")